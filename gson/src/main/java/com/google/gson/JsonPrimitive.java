...
public final class JsonPrimitive extends JsonElement {
    ...
    public int hashCode() {
        ...
        if (value instanceof Number) {
            double tem = getAsNumber().doubleValue();
            long value = Double.doubleToLongBits(tem);
            return (int) (value ^ (value >>> 32));
        }
    ...
    private static boolean isIntegral(JsonPrimitive primitive) {
        if (primitive.value instanceof Number) {
            ...
            return number instanceof BigInteger || number instanceof Long || number instanceof Integer
                    || number instanceof Short || number instanceof Byte || isLongLazilyParsedNumber(number);
        }
        return false;
    }

    private static boolean isLongLazilyParsedNumber(Number number) {
        if (number instanceof LazilyParsedNumber) {
            return Primitives.isLong(((LazilyParsedNumber) number).toString());
